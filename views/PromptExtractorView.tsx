...
interface PromptExtractorViewProps {
  t: TFunction;
}

type ExtractionMode = 'descriptive' | 'concise';

export const PromptExtractorView: React.FC<PromptExtractorViewProps> = ({ t }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-4 md:p-8">
            ...
            <div className="flex flex-col gap-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm">
                <h2 className="text-3xl font-bold text-brand-primary dark:text-white">{t('prompt_extractor_title')}</h2>
                
                <div>
                    <label className="block text-lg font-semibold text-brand-primary dark:text-gray-300 mb-2">{t('prompt_extractor_image_label')}</label>
                    <ImageUploader images={baseImage} setImages={setBaseImage} maxFiles={1} t={t} descriptionKey="prompt_extractor_image_desc" />
                </div>
                ...
                >
                    {isLoading ? '...' : t('extract_prompt_button')}
                </ActionButton>
            </div>
            ...
            <div className="bg-white/70 dark:bg-gray-900/50 p-4 rounded-2xl shadow-inner border border-gray-200/80 dark:border-gray-700 min-h-[400px] lg:min-h-0 flex items-center justify-center">
                {isLoading ? (
                     <div className="text-center text-gray-500 dark:text-gray-400">
                        <SpinnerIcon />
                        <p className="mt-4 text-lg font-semibold text-brand-primary dark:text-brand-accent">{t('extracting_prompt_title')}</p>
                        <p className="text-sm">{t('extracting_prompt_desc')}</p>
                     </div>
                ) : (
                    <PromptResultDisplay text={extractedText} t={t} outputLanguage={extractionLanguage} initialDescKey="initial_desc_prompt_extractor" />
                )}
            </div>
        </div>
    );
